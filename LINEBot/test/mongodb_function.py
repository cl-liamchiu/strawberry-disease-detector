...
def read_many_datas():
    ...
    for data in col.find():
        ...
        if dicMemberCheck('events', data):
            if dicMemberCheck('postback', data['events'][0]):
                if dicMemberCheck('data', data['events'][0]['postback']):
                    print(data['events'][0]['postback']['data'])
                    data_list.append(data['events'][0]['postback']['data'])
        else:
            continue
...
if __name__ == '__main__':
    ...
    # print("test")
    my_query = {'source': {'type': 'user'}}

    # for data in col.find(my_query):
    #     print(data, "\n")
    # new_values = { "$set": { "postback": {"data": "hello" } }}
    # col.update_one(my_query, new_values)
    result = col.find_one(my_query)
    print(result)
    delete_all_data()
